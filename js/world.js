...
function pickItem(it){const p=player;burst(it.x,it.y,it.col,16,4);
  ...
  if(it.type==='heal'){p.hp=Math.min(p.maxhp,p.hp+25);floatText(p.x,p.y-22,tr('+25 HP'),'#ff5fa2');Fx.sfx('tone',440,900,.25,'sine',.09);}
  ...
  else if(it.type==='rage'){p.rageT=540;Fx.sfx('tone',180,680,.35,'sawtooth',.11);floatText(p.x,p.y-22,tr('OVERDRIVE'),'#d97757');}
}
